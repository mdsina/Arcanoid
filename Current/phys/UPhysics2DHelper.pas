...
procedure BuildEdgeShapeCurve(pts: array of TPointF; cnt: Int32; bd: Tb2Body;
...
procedure BuildRope(pts: array of TPointF; cnt: Int32; world: Tb2World; bodies: TList = nil;
...
begin
   ...
   BuildEdgeShapeCurve(Walls, 5, Result, 0.1, 0, curve_tension); // Change tension to make the box rectangle
end;
...
procedure calc_curve_bezier(const pts: array of TPointF; tension: Single;
...
procedure GenCurvePoints(const points: array of TPointF; count: Int32; tension: Single = 0.5);
var
   i, len_pt: Int32;
   x1, x2, y1, y2: Single;
   pt: array of TPointF;
begin
   ...
   begin
      calc_curve_bezier(points, tension, x1, y1, x2, y2);
      pt[3 * i + 2].X := x1;
      pt[3 * i + 2].Y := y1;
      pt[3 * i + 3] := points[i + 1];
      pt[3 * i + 4].X := x2;
...
begin
   ...
   GenCurvePoints(pts, cnt, curve_tension);
...
begin
   pts[0] := p1;
   pts[1] := p2;
   BuildRope(pts, 2, world, bodies, body1, body2, 1, max_segment,
